...
const PlanCard = ({ title, price, duration, features, isPopular }) => (
  <View className={`bg-white dark:bg-slate-800 rounded-3xl p-6 mb-6 shadow-sm border ${isPopular ? 'border-primaryPink border-2' : 'border-gray-100 dark:border-slate-700'}`}>
    {isPopular && (
      <View className="absolute -top-3 left-6 bg-primaryPink px-3 py-1 rounded-full">
    ...
    <View className="mt-6 space-y-4">
      {features.map((feature, index) => (
        <View key={index} className="flex-row items-center">
          <Feather name="check-circle" size={18} color="#FF8383" />
          <Text className="ml-3 text-gray-600 dark:text-gray-300">{feature}</Text>
        </View>
      ))}
    </View>
    
    <TouchableOpacity className={`mt-8 py-4 rounded-2xl items-center ${isPopular ? 'bg-primaryPink' : 'bg-gray-100 dark:bg-slate-700'}`}>
